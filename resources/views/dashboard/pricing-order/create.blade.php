...
@section('container')
	<h2 class="mb-3 fs-sm-3">Tambah Data Harga Order</h2>
	...
	<div class="col-lg-8 mb-5 p-0">
		<form action="{{ route('pricing-order-transport.store') }}" method="post">
			@csrf
			<div class="mb-3">
				...
				@if ($location_starts->count() > 0)
					<select class="form-select" name="location_start_id" id="location_start_id" required>
						...
						@foreach ($location_starts as $location_start)
							@if (old("location_start_id") == $location_start->id)
				...
				@if ($location_finishs->count() > 0)
					<select class="form-select" name="location_finish_id" id="location_finish_id" required>
						...
						@foreach ($location_finishs as $location_finish)
							@if (old("location_finish_id") == $location_finish->id)
			...
			<div class="mb-3">
				...
				<input type="number" name="distance" class="form-control @error('distance') is-invalid @enderror" id="distance" value="{{ old("distance") }}" required>
				@error('distance')
					<div class="invalid-feedback">
						{!! $message !!}
					</div>
