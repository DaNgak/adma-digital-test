...
@section('container')
    ...
    <div class="col-xl-8 p-0 mb-3 mb-md-5">
        ...
        @if ($orderTransport->status == "ongoing")    
            <form action="{{ route('user.order.cancel', $orderTransport->id) }}" class="d-inline w-100 w-lg-auto mb-3 mb-lg-0" method="post">
                @csrf
                @method("put")
                <button onclick="return confirm('Konfirmasi batalkan pesanan ?')" class="btn btn-danger w-100 w-lg-auto">Batalkan Pesanan</button>
            </form>
        @endif
    </div>
