...
@section('container')
    ...
    @if (!isset($search_key))
        @if ($locations->count() > 0)
            <div class="d-flex mb-3">
                <form action="{{ route('location.trash.search') }}" method="post" class="d-inline-block navbar-search w-100">
                    @csrf
                    @method("post")
            ...
            <div class="table-responsive mb-5">
                <table class="table table-striped table-sm" >
                    <caption>Tabel Lokasi Sampah</caption>
                    <thead>
                        <tr>
                        ...
                        @foreach ($locations as $location)        
                            <tr>
                                ...
                                <td class="d-flex justify-content-center">
                                    <form action="{{ route('location.trash.restore', $location->id) }}" class="d-inline" method="post">
                                        @csrf
                                        @method("put")
                                        <button onclick="return confirm('Konfirmasi restore data ?')" class="btn btn-warning mr-2">Restore</button>
                                    </form>
                                    <form action="{{ route('location.trash.destroy', $location->id) }}" class="d-inline" method="post">
                                        @csrf
                                        @method("delete")
        ...
        @else
            <div class="d-flex">
                <h3>Tidak ada Data Lokasi Sampah</h3>
            </div> 
        @endif
    @endif
    ...
    @isset($search_key)
        ...
        @else
            <div class="d-flex">
                <h3>Tidak ada Data {{ $search_key }} pada Tabel Lokasi Sampah</h3>
            </div> 
        @endif
    @endisset
@endsection